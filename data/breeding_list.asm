BreedingList:
;Which Pokemon is the baby?
db BULBASAUR  ; Bulbasaur
db BULBASAUR  ; Ivysaur
db BULBASAUR  ; Venusaur
db CHARMANDER ; Charmander
db CHARMANDER ; Charmeleon
db CHARMANDER ; Charizard
db SQUIRTLE   ; Squirtle
db SQUIRTLE   ; Wartortle
db SQUIRTLE   ; Blastoise
db CATERPIE   ; Caterpie
db CATERPIE   ; Metapod
db CATERPIE   ; Butterfree
db WEEDLE     ; Weedle
db WEEDLE     ; Kakuna
db WEEDLE     ; Beedrill
db PIDGEY     ; Pidgey
db PIDGEY     ; Pidgeotto
db PIDGEY     ; Pidgeot
db RATTATA    ; Rattata
db RATTATA    ; Raticate
db SPEAROW    ; Spearow
db SPEAROW    ; Fearow
db EKANS      ; Ekans
db EKANS      ; Arbok
db PIKACHU      ; Pikachu
db PIKACHU      ; Raichu
db SANDSHREW  ; Sandshrew
db SANDSHREW  ; Sandslash
db NIDORAN_F  ; Nidoran F
db NIDORAN_F  ; Nidorina
db NIDORAN_F  ; Nidoqueen
db NIDORAN_M  ; Nidoran M
db NIDORAN_M  ; Nidorino
db NIDORAN_M  ; Nidoking
db CLEFAIRY     ; Clefairy
db CLEFAIRY     ; Clefable
db VULPIX     ; Vulpix
db VULPIX     ; Ninetails
db JIGGLYPUFF  ; Jigglypuff
db JIGGLYPUFF  ; Wigglytuff
db ZUBAT      ; Zubat
db ZUBAT      ; Golbat
db ODDISH     ; Oddish
db ODDISH     ; Gloom
db ODDISH     ; Vileplume
db PARAS      ; Paras
db PARAS      ; Parasect
db VENONAT    ; Venonat
db VENONAT    ; Venomoth
db DIGLETT    ; Diglett
db DIGLETT    ; Dugtrio
db MEOWTH     ; Meowth
db MEOWTH     ; Persian
db PSYDUCK    ; Psyduck
db PSYDUCK    ; Golduck
db MANKEY     ; Mankey
db MANKEY     ; Primeape
db GROWLITHE  ; Growlithe
db GROWLITHE  ; Arcanine
db POLIWAG    ; Poliwag
db POLIWAG    ; Poliwhirl
db POLIWAG    ; Poliwrath
db ABRA       ; Abra
db ABRA       ; Kadabra
db ABRA       ; Alakazam
db MACHOP     ; Machop
db MACHOP     ; Machoke
db MACHOP     ; Machamp
db BELLSPROUT ; Bellsprout
db BELLSPROUT ; Weepinbell
db BELLSPROUT ; Victreebel
db TENTACOOL  ; Tentacool
db TENTACOOL  ; Tentacruel
db GEODUDE    ; Geodude
db GEODUDE    ; Graveler
db GEODUDE    ; Golem
db PONYTA     ; Ponyta
db PONYTA     ; Rapidash
db SLOWPOKE   ; Slowpoke
db SLOWPOKE   ; Slowbro
db MAGNEMITE  ; Magnemite
db MAGNEMITE  ; Magneton
db FARFETCHD  ; Farfetch'd
db DODUO      ; Doduo
db DODUO      ; Dodrio
db SEEL       ; Seel
db SEEL       ; Dewgong
db GRIMER     ; Grimer
db GRIMER     ; Muk
db SHELLDER   ; Shellder
db SHELLDER   ; Cloyster
db GASTLY     ; Gastly
db GASTLY     ; Haunter
db GASTLY     ; Gengar
db ONIX       ; Onix
db DROWZEE    ; Drowzee
db DROWZEE    ; Hypno
db KRABBY     ; Krabby
db KRABBY     ; Kingler
db VOLTORB    ; Voltorb
db VOLTORB    ; Electrode
db EXEGGCUTE  ; Exeggcute
db EXEGGCUTE  ; Exeggutor
db CUBONE     ; Cubone
db CUBONE     ; Marowak
db MANKEY    ; Hitmonlee
db MANKEY    ; Hitmonchan
db LICKITUNG  ; Lickitung
db KOFFING    ; Koffing
db KOFFING    ; Weezing
db RHYHORN    ; Rhyhorn
db RHYHORN    ; Rhydon
db CHANSEY    ; Chansey
db TANGELA    ; Tangela
db KANGASKHAN ; Kangaskhan
db HORSEA     ; Horsea
db HORSEA     ; Seadra
db GOLDEEN    ; Goldeen
db GOLDEEN    ; Seaking
db STARYU     ; Staryu
db STARYU     ; Starmie
db MR_MIME    ; Mr. Mime
db SCYTHER    ; Scyther
db JYNX   ; Jynx
db ELECTABUZZ     ; Electabuzz
db MAGMAR      ; Magmar
db PINSIR     ; Pinsir
db TAUROS     ; Tauros
db MAGIKARP   ; Magikarp
db MAGIKARP   ; Gyarados
db LAPRAS     ; Lapras
db DITTO      ; Ditto
db EEVEE      ; Eevee
db EEVEE      ; Vaporeon
db EEVEE      ; Jolteon
db EEVEE      ; Flareon
db PORYGON    ; Porygon
db OMANYTE    ; Omanyte
db OMANYTE    ; Omastar
db KABUTO     ; Kabuto
db KABUTO     ; Kabutops
db AERODACTYL ; Aerodactyl
db SNORLAX   ; Snorlax
db ARTICUNO   ; Articuno
db ZAPDOS     ; Zapdos
db MOLTRES    ; Moltres
db DRATINI    ; Dratini
db DRATINI    ; Dragonair
db DRATINI    ; Dragonite
db MEWTWO     ; Mewtwo
db MEW        ; Mew
db ARTICUNO      ; Lugia
db GROWLITHE   ; Houndour
db GROWLITHE   ; Houndoom
db SPEAROW    ; Murkrow
db SPEAROW    ; Honchkrow
db PINSIR  ; Heracross
db EEVEE      ; Espeon
db EEVEE      ; Umbreon
db EEVEE      ; Glaceon
db EEVEE      ; Leafeon
db EEVEE      ; Sylveon
db SCYTHER    ; Scizor
db ONIX       ; Steelix
db ZUBAT      ; Crobat
db POLIWAG    ; Politoed
db SLOWPOKE   ; Slowking
db ODDISH     ; Bellossom
db HORSEA     ; Kingdra
db CHANSEY    ; Blissey
db PORYGON    ; Porygon 2
db PORYGON    ; Porygon Z
db MAGMAR      ; Magmortar
db ELECTABUZZ     ; Electivire
db MAGNEMITE  ; Magnezone
db RHYHORN    ; Rhyperior
db TANGELA    ; Tangrowth
db LICKITUNG  ; Lickilicky
db CLEFAIRY     ; Togepi
db CLEFAIRY     ; Togetic
db CLEFAIRY     ; Togekiss
db PERSIAN    ; Sneasel
db PERSIAN    ; Weavile
db FEAROW   ; Skarmory
db GASTLY ; Misdreavus
db GASTLY ; Mismagius
db TAUROS    ; Miltank
db MAGNEMITE   ; Chinchou
db MAGNEMITE   ; Lanturn
db MAGMAR     ; Slugma
db MAGMAR     ; Magcargo
db CHARIZARD    ; Torkoal
db DRAGONITE     ; Latias
db DRAGONITE     ; Latios
db MANKEY    ; Hitmontop
db MANKEY    ; Tyrogue
db PIKACHU      ; Pichu
db CLEFAIRY     ; Cleffa
db JIGGLYPUFF  ; Igglybuff
db JYNX   ; Smoochum
db ELECTABUZZ     ; Elekid
db MAGMAR      ; Magby
db MR_MIME    ; Mime Jr
db CHANSEY    ; Happiny
db SNORLAX   ; Munchlax
db RATICATE  ; Zigzagoon
db RATICATE  ; Linoone
db MOLTRES      ; Ho-oh
